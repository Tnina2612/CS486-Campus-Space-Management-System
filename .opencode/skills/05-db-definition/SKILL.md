...
9. Table-Level vs. Column-Level Constraints: In SQL Server, `CHECK` constraints that evaluate multiple columns simultaneously (e.g., ensuring `end_time` is greater than `start_time`) MUST be declared as table-level constraints at the end of the `CREATE TABLE` definition. Do NOT write them inline as column-level constraints, as this will trigger Msg 8141 and cause table creation to fail.
